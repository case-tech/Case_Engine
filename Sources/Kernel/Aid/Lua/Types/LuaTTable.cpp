        ...
        namespace LuaCpp
        {
            namespace Types
            {
                ...
                void LuaTTable::PopValue(LuaState& L_a, int idx_a)
                {
                    if (idx_a < 0) 
                    {
                        idx_a = lua_gettop(L) + idx_a + 1;
                    }

                    if (idx_a <= 0) 
                    {
                        throw std::invalid_argument("The stack position " + std::to_string(idx_a) + " is invalid.");
                    }
                    ...
                    if (lua_istable(L_a, idx_a) == 1) 
                    {
                        ...
                        while (lua_next(L_a, idx_a) != 0) 
                        {
                            ...
                            default: 
                            {
                                field_ = std::make_shared<LuaTString>(lua_typename(L, lua_type(L_a, -1)));
                                break;
                            }
                            }
                            ...
                            {
                                SetValue(Table::Key(lua_tostring(L_a, -2)), field_);
                            } 
                            
                            else 
                            {
                                SetValue(Table::Key(lua_tointeger(L_a, -2)), field_);
                            }
                    ...
                    {
                        throw std::invalid_argument("The value at the index " + std::to_string(idx_a) + " is not a LUA_TTABLE");
                    }
                }
                ...
                void LuaTTable::SetValue(Table::Key key_a, std::shared_ptr<LuaType> value_a)
                {
                    if (!key_a.IsNumber()) 
                    {
                        is_array_ = false;
