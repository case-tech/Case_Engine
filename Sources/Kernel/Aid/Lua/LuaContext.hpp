...
namespace CE_Kernel
{
    namespace Aid
    {
        namespace LuaCpp
        {
            typedef std::map<std::string, std::shared_ptr<Types::LuaType>> LuaEnvironment;

            class LuaContext
            {
            public:
                LuaContext() : registry_(), libraries_(), 
                               global_environment_(), built_in_functions_() {};
                ~LuaContext() {};

                std::unique_ptr<Types::LuaState> NewState();
                std::unique_ptr<Types::LuaState> NewStateFor(const std::string& name_a);
                std::unique_ptr<Types::LuaState> NewStateFor(const std::string& name_a, 
                                                             const LuaEnvironment& env_a);
                void CompileString(const std::string& name_a, const std::string& code_a);
                void CompileString(const std::string& name_a, 
                        const std::string& code_a, bool recompile_a);

                void CompileFile(const std::string& name_a, const std::string& file_name_a,
                        bool recompile_a);

                void CompileFolder(const std::string& path_a);
                void CompileFolder(const std::string& path_a, const std::string& prfix_a);
                void CompileFolder(const std::string& path_a, const std::string& prefix_a, 
                        bool recompile_a);
                ...
                void SetBuiltInFnc(const std::string& fnc_name_a, lua_CFunction cfunction_a);
                void SetBuiltInFnc(const std::string& fnc_name_a, lua_CFunction cfunction_a, 
                        bool replace_a);

                bool Exists_buildInFnc(Types::LuaState& L_a, const std::string& fnc_name_a);
                ...
                std::shared_ptr<Types::LuaType>& GetGlobalVariable(const std::string& name_a);
                void AddHook(lua_Hook hook_func_a, const std::string& hook_type_a, 
                        const int count_a = 0);
            ...
            private:
                ...
                std::map<std::string, std::shared_ptr<Registry::LuaLibrary>> libraries_;
                std::vector<std::tuple<std::string, int, lua_Hook>> hooks_;
                LuaEnvironment global_environment_;
                std::map<std::string, LuaCpp::Registry::LuaCFunction> built_in_functions_;
            }
        }
    }
}
