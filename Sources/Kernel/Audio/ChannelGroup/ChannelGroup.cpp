...
namespace CE_Kernel
{
    namespace Audio
    {
	    ...
	    {
	    	StopAll();
		if (group_) FMOD_ChannelGroup_Release(group_);
	    }
			    ...
			    std::shared_ptr<CE_Kernel::Audio::Channel::Channel> channel_a)
	    {
	    	if (group_ && channel_a)
		{
		   FMOD_RESULT result_ =  FMOD_Channel_SetChannelGroup(channel_a->GetPtr(), group_);
	    ...
	    {
	    	if (group_) FMOD_ChannelGroup_SetVolume(group_, volume_a);
	    }

	    void ChannelGroup::StopAll()
	    {
	    	if (group_) FMOD_ChannelGroup_Stop(group_);
		channels_.clear();
	    }
