...
namespace AZGUIZMO_NAMESPACE
{
    ...
    float matrix_t::Inverse(const matrix_t& srcMatrix, bool affine)
    {
        ...
        if (affine)
        {
            ...
            m[2][2] = (srcMatrix.m[0][0] * srcMatrix.m[1][1]
                      ...
                      * s;
            m[3][0] = -(m[0][0] * srcMatrix.m[3][0]
                        + m[1][0] * srcMatrix.m[3][1]
                        + m[2][0] * srcMatrix.m[3][2]);
            m[3][1] = -(m[0][1] * srcMatrix.m[3][0]
                        + m[1][1] * srcMatrix.m[3][1]
                        + m[2][1] * srcMatrix.m[3][2]);
            m[3][2] = -(m[0][2] * srcMatrix.m[3][0]
                        + m[1][2] * srcMatrix.m[3][1]
        ...
        } else
        {
            ...
            m16[10] = (tmp[2] * src[12] + tmp[7] * src[13] + tmp[10] * src[15])
                      - (tmp[3] * src[12] + tmp[6] * src[13]
                         + tmp[11] * src[15]);
            m16[11] = (tmp[5] * src[12] + tmp[8] * src[13] + tmp[11] * src[14])
                      - (tmp[4] * src[12] + tmp[9] * src[13]
                         + tmp[10] * src[14]);
    ...
    Style::Style()
    {
        ...
        Colors[DIRECTION_X] = AzVec4(0.666f, 0.000f, 0.000f, 1.000f);
        Colors[DIRECTION_Y] = AzVec4(0.000f, 0.666f, 0.000f, 1.000f);
        Colors[DIRECTION_Z] = AzVec4(0.000f, 0.000f, 0.666f, 1.000f);
        Colors[PLANE_X] = AzVec4(0.666f, 0.000f, 0.000f, 0.380f);
        Colors[PLANE_Y] = AzVec4(0.000f, 0.666f, 0.000f, 0.380f);
        Colors[PLANE_Z] = AzVec4(0.000f, 0.000f, 0.666f, 0.380f);
        Colors[SELECTION] = AzVec4(1.000f, 0.500f, 0.062f, 0.541f);
        Colors[INACTIVE] = AzVec4(0.600f, 0.600f, 0.600f, 0.600f);
        Colors[TRANSLATION_LINE] = AzVec4(0.666f, 0.666f, 0.666f, 0.666f);
        Colors[SCALE_LINE] = AzVec4(0.250f, 0.250f, 0.250f, 1.000f);
        Colors[ROTATION_USING_BORDER] = AzVec4(1.000f, 0.500f, 0.062f, 1.000f);
        Colors[ROTATION_USING_FILL] = AzVec4(1.000f, 0.500f, 0.062f, 0.500f);
        Colors[HATCHED_AXIS_LINES] = AzVec4(0.000f, 0.000f, 0.000f, 0.500f);
        Colors[TEXT] = AzVec4(1.000f, 1.000f, 1.000f, 1.000f);
        Colors[TEXT_SHADOW] = AzVec4(0.000f, 0.000f, 0.000f, 1.000f);
    }
    ...
    struct Context
    {
        ...
        AzVec2 mScreenSquareCenter;
        AzVec2 mScreenSquareMin;
        AzVec2 mScreenSquareMax;
        ...
        AzWindow* mAlternativeWindow = nullptr;
        AzVector<AzID> mIDStack;
        AzID mEditingID = -1;
    ...
    static const float quadUV[8] = {quadMin,
                                    quadMin,
                                    quadMin,
                                    quadMax,
                                    quadMax,
    ...
    static AzU32 GetColorU32(int idx)
    {
        AZ_ASSERT(idx < COLOR::COUNT);
        return Az::ColorConvertFloat4ToU32(gContext.mStyle.Colors[idx]);
    }

    static AzVec2 worldToPos(const vec_t& worldPos,
                             const matrix_t& mat,
                             AzVec2 position = AzVec2(gContext.mX, gContext.mY),
                             AzVec2 size = AzVec2(gContext.mWidth,
                                                  gContext.mHeight))
    {
        vec_t trans;
        ...
        trans.x += position.x;
        trans.y += position.y;
        return AzVec2(trans.x, trans.y);
    }

    static void ComputeCameraRay(vec_t& rayOrigin,
                                 vec_t& rayDir,
                                 AzVec2 position = AzVec2(gContext.mX,
                                                          gContext.mY),
                                 AzVec2 size = AzVec2(gContext.mWidth,
                                                      gContext.mHeight))
    {
                                           ...
                                           const bool localCoordinates = false)
    {
        ...
        const matrix_t& mvp = localCoordinates ? gContext.mMVPLocal
                                               : gContext.mMVP;
    ...
    static bool IsInContextRect(AzVec2 p)
    {
    ...
    {
        AzContext& g = *Az::GetCurrentContext();
        AzWindow* window = Az::FindWindowByName(gContext.mDrawList->_OwnerName);
        ...
        if (g.HoveredWindow != NULL)
            return false;
        if (Az::IsMouseHoveringRect(window->InnerRect.Min,
    ...
    void SetDrawlist(AzDrawList* drawlist)
    {
        gContext.mDrawList = drawlist ? drawlist : Az::GetWindowDrawList();
    }

    void SetAzContext(AzContext* ctx)
    {
    ...
    void BeginFrame()
    {
        const AzU32 flags = AzWindowFlags_NoTitleBar | AzWindowFlags_NoResize
                            | AzWindowFlags_NoScrollbar | AzWindowFlags_NoInputs
                            | AzWindowFlags_NoSavedSettings
                            | AzWindowFlags_NoFocusOnAppearing
                            | AzWindowFlags_NoBringToFrontOnFocus;

#ifdef AZ_HAS_VIEWPORT
        Az::SetNextWindowSize(Az::GetMainViewport()->Size);
        Az::SetNextWindowPos(Az::GetMainViewport()->Pos);
#else
        AzIO& io = Az::GetIO();
        Az::SetNextWindowSize(io.DisplaySize);
        Az::SetNextWindowPos(AzVec2(0, 0));
#endif

        Az::PushStyleColor(AzCol_WindowBg, 0);
        Az::PushStyleColor(AzCol_Border, 0);
        Az::PushStyleVar(AzStyleVar_WindowRounding, 0.0f);

        Az::Begin("gizmo", NULL, flags);
        gContext.mDrawList = Az::GetWindowDrawList();
                               ...
                               MODE mode)
    {
        ...
        float rightLength = GetSegmentLengthClipSpace(makeVect(0.f, 0.f),
                                                      rightViewInverse);
        gContext.mScreenFactor = gContext.mGizmoSizeClipSpace / rightLength;

        AzVec2 centerSSpace = worldToPos(makeVect(0.f, 0.f), gContext.mMVP);
        gContext.mScreenSquareCenter = centerSSpace;
        gContext.mScreenSquareMin = AzVec2(centerSSpace.x - 10.f,
                                           centerSSpace.y - 10.f);
        gContext.mScreenSquareMax = AzVec2(centerSSpace.x + 10.f,
                                           centerSSpace.y + 10.f);

        ComputeCameraRay(gContext.mRayOrigin, gContext.mRayVector);
    }

    static void ComputeColors(AzU32* colors, int type, OPERATION operation)
    {
        if (gContext.mbEnable)
        {
            AzU32 selectionColor = GetColorU32(SELECTION);

            switch (operation)
            {
            case TRANSLATE:
                colors[0] = (type == MT_MOVE_SCREEN) ? selectionColor
                                                     : AZ_COL32_WHITE;
                for (int i = 0; i < 3; i++)
                {
            ...
            case SCALEU:
            case SCALE:
                colors[0] = (type == MT_SCALE_XYZ) ? selectionColor
                                                   : AZ_COL32_WHITE;
                for (int i = 0; i < 3; i++)
                {
        ...
        {
            AzU32 inactiveColor = GetColorU32(INACTIVE);
            for (int i = 0; i < 7; i++)
            {
            ...
            const bool localCoordinates = false)
    {
        ...
        } else
        {
            ...
            float paraSurf =
                    GetParallelogram(makeVect(0.f, 0.f, 0.f),
    ...
    static float ComputeAngleOnPlan()
    {
        ...
        vec_t localPos = Normalized(gContext.mRayOrigin
                                    + gContext.mRayVector * len
    ...
    static void DrawRotationGizmo(OPERATION op, int type)
    {
        ...
        AzDrawList* drawList = gContext.mDrawList;

        bool isMultipleAxesMasked = (gContext.mAxisMask
                                     & (gContext.mAxisMask - 1))
                                    != 0;
        ...
        {
            cameraToModelNormalized = Normalized(gContext.mModel.v.position
                                                 - gContext.mCameraEye);
        }
        ...
        for (int axis = 0; axis < 3; axis++)
        {
            ...
            const bool usingAxis = (gContext.mbUsing
                                    && type == MT_ROTATE_Z - axis);
            const int circleMul = (hasRSC && !usingAxis) ? 1 : 2;

            AzVec2* circlePos = (AzVec2*)alloca(
                    sizeof(AzVec2) * (circleMul * halfCircleSegmentCount + 1));
            ...
            float radiusAxis = sqrtf(
                    (AzLengthSqr(worldToPos(gContext.mModel.v.position,
        ...
        {
            drawList->AddCircle(worldToPos(gContext.mModel.v.position,
                                           gContext.mViewProjection),
        ...
        if (gContext.mbUsing && (gContext.GetCurrentID() == gContext.mEditingID)
            && IsRotateType(type))
        {
            AzVec2 circlePos[halfCircleSegmentCount + 1];
            ...
            for (unsigned int i = 1; i < halfCircleSegmentCount + 1; i++)
            {
                float ng = gContext.mRotationAngle
                           * ((float)(i - 1)
                              / (float)(halfCircleSegmentCount - 1));
            ...
            AzVec2 destinationPosOnScreen = circlePos[1];
            char tmps[512];
            AzFormatString(tmps,
                           sizeof(tmps),
            ...
            drawList->AddText(AzVec2(destinationPosOnScreen.x + 15,
                                     destinationPosOnScreen.y + 15),
                              GetColorU32(TEXT_SHADOW),
                              tmps);
            drawList->AddText(AzVec2(destinationPosOnScreen.x + 14,
    ...
    static void DrawHatchedAxis(const vec_t& axis)
    {
        ...
        for (int j = 1; j < 10; j++)
        {
            AzVec2 baseSSpace2 = worldToPos(axis * 0.05f * (float)(j * 2)
                                                    * gContext.mScreenFactor,
                                            gContext.mMVP);
            AzVec2 worldDirSSpace2 = worldToPos(
                ...
                if (belowAxisLimit)
                {
                    ...
                    AzVec2 baseSSpace =
                            worldToPos(dirAxis * 0.1f * gContext.mScreenFactor,
                                       gContext.mMVP);
                    AzVec2 worldDirSSpaceNoScale = worldToPos(
                            dirAxis * markerScale * gContext.mScreenFactor,
                            gContext.mMVP);
                    AzVec2 worldDirSSpace =
                    ...
                    if (gContext.mbUsing
                        && (gContext.GetCurrentID() == gContext.mEditingID))
                    {
                        AzU32 scaleLineColor = GetColorU32(SCALE_LINE);
    ...
    static void DrawScaleUniveralGizmo(OPERATION op, int type)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (!gContext.mbUsing || usingAxis)
            {
               ...
               if (gContext.mbUsing && (gContext.GetCurrentID() == gContext.mEditingID))
               {
                  drawList->AddLine(baseSSpace, worldDirSSpaceNoScale, AZ_COL32(0x40, 0x40, 0x40, 0xFF), 3.f);
                  drawList->AddCircleFilled(worldDirSSpaceNoScale, 6.f, AZ_COL32(0x40, 0x40, 0x40, 0xFF));
               }
        ...
        drawList->AddCircle(gContext.mScreenSquareCenter,
                            20.f,
                            colors[0],
                            32,
                            gContext.mStyle.CenterCircleSize);
    ...
    static void DrawTranslationGizmo(OPERATION op, int type)
    {
        ...
        const AzVec2 origin = worldToPos(gContext.mModel.v.position,
                                         gContext.mViewProjection);
        ...
        for (int i = 0; i < 3; ++i)
        {
            ...
            if (!gContext.mbUsing
                || (gContext.mbUsing && type == MT_MOVE_X + i))
            {
                if (belowAxisLimit
                    && Intersects(op, static_cast<OPERATION>(TRANSLATE_X << i)))
                {
                    AzVec2 baseSSpace =
                            worldToPos(dirAxis * 0.1f * gContext.mScreenFactor,
                                       gContext.mMVP);
                    AzVec2 worldDirSSpace =
                            worldToPos(dirAxis * gContext.mScreenFactor,
                                       gContext.mMVP);
                    ...
                    AzVec2 dir(origin - worldDirSSpace);

                    float d = sqrtf(AzLengthSqr(dir));
                    dir /= d;
                    dir *= gContext.mStyle.TranslationLineArrowSize;

                    AzVec2 ortogonalDir(dir.y, -dir.x);
                    AzVec2 a(worldDirSSpace + dir);
            ...
            if (!gContext.mbUsing
                || (gContext.mbUsing && type == MT_MOVE_YZ + i))
            {
                if (belowPlaneLimit && Contains(op, TRANSLATE_PLANS[i]))
                {
                    AzVec2 screenQuadPts[4];
                    for (int j = 0; j < 4; ++j)
                    {
                    ...
                    drawList->AddPolyline(screenQuadPts,
                                          ...
                                          true,
                                          1.0f);
                    drawList->AddConvexPolyFilled(screenQuadPts,
                                                  4,
                                                  colors[i + 4]);
                }
            }
        ...
        {
            AzU32 translationLineColor = GetColorU32(TRANSLATION_LINE);

            AzVec2 sourcePosOnScreen = worldToPos(gContext.mMatrixOrigin,
                                                  gContext.mViewProjection);
            AzVec2 destinationPosOnScreen =
                    worldToPos(gContext.mModel.v.position,
                               gContext.mViewProjection);
            ...
            drawList->AddCircle(destinationPosOnScreen,
                                6.f,
                                translationLineColor);
            drawList->AddLine(AzVec2(sourcePosOnScreen.x + dif.x,
                                     sourcePosOnScreen.y + dif.y),
                              AzVec2(destinationPosOnScreen.x - dif.x,
    ...
    {
        if (Az::IsMouseClicked(0) && !Az::IsAnyItemHovered()
            && !Az::IsAnyItemActive())
        {
            return true;
        }
                                         ...
                                         OPERATION operation)
    {
        ...
        if (!gContext.mbUsingBounds)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                float dt = fabsf(
                        Dot(Normalized(gContext.mCameraEye
        ...
        for (unsigned int axisIndex = 0; axisIndex < numAxes; ++axisIndex)
        {
            ...
            matrix_t boundsMVP = gContext.mModelSource
                                 * gContext.mViewProjection;
            for (int i = 0; i < 4; i++)
            {
                AzVec2 worldBound1 = worldToPos(aabb[i], boundsMVP);
                AzVec2 worldBound2 = worldToPos(aabb[(i + 1) % 4], boundsMVP);
                ...
                float boundDistance = sqrtf(
                        AzLengthSqr(worldBound1 - worldBound2));
                ...
                for (int j = 0; j < stepCount; j++)
                {
                    ...
                    AzVec2 worldBoundSS1 = AzLerp(worldBound1,
                                                  worldBound2,
                                                  AzVec2(t1, t1));
                    AzVec2 worldBoundSS2 = AzLerp(worldBound1,
                                                  worldBound2,
                                                  AzVec2(t2, t2));

                    drawList->AddLine(worldBoundSS1,
                                      worldBoundSS2,
                                      AZ_COL32(0xAA, 0xAA, 0xAA, 0)
                                              + anchorAlpha,
                                      2.f);
                }
                vec_t midPoint = (aabb[i] + aabb[(i + 1) % 4]) * 0.5f;
                AzVec2 midBound = worldToPos(midPoint, boundsMVP);
                static const float AnchorBigRadius = 8.f;
                static const float AnchorSmallRadius = 6.f;
                bool overBigAnchor = AzLengthSqr(worldBound1 - io.MousePos)
                                     <= (AnchorBigRadius * AnchorBigRadius);
                bool overSmallAnchor = AzLengthSqr(midBound - io.MousePos)
                                       <= (AnchorBigRadius * AnchorBigRadius);
                ...
                unsigned int bigAnchorColor =
                        overBigAnchor
                                ? selectionColor
                                : (AZ_COL32(0xAA, 0xAA, 0xAA, 0) + anchorAlpha);
                    ...
                    && overBigAnchor && CanActivate())
                {
                    gContext.mBoundsPivot.TransformPoint(aabb[(i + 2) % 4],
                                                         gContext.mModelSource);
                    gContext.mBoundsAnchor
                            .TransformPoint(aabb[i], gContext.mModelSource);
                    gContext.mBoundsPlan = BuildPlan(gContext.mBoundsAnchor,
                                                     bestAxisWorldDirection);
                ...
                {
                    vec_t midPointOpposite = (aabb[(i + 2) % 4]
                                              + aabb[(i + 3) % 4])
                                             * 0.5f;
                    gContext.mBoundsPivot.TransformPoint(midPointOpposite,
                                                         gContext.mModelSource);
                    gContext.mBoundsAnchor
                            .TransformPoint(midPoint, gContext.mModelSource);
                    gContext.mBoundsPlan = BuildPlan(gContext.mBoundsAnchor,
                                                     bestAxisWorldDirection);
                    gContext.mBoundsBestAxis = bestAxis;
                ...
                && (gContext.GetCurrentID() == gContext.mEditingID))
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    float boundSize = bounds[axisIndex1 + 3]
                                      - bounds[axisIndex1];
                ...
                matrix_t res = preScale * scale * postScale
                               * gContext.mBoundsMatrix;
                *matrix = res;
    ...
    static int GetScaleType(OPERATION op)
    {
        ...
        for (int i = 0; i < 3 && type == MT_NONE; i++)
        {
            ...
            const float startOffset =
                    Contains(op, static_cast<OPERATION>(TRANSLATE_X << i))
                            ? 1.0f
                            : 0.1f;
            const float endOffset = Contains(op,
                                             static_cast<OPERATION>(TRANSLATE_X
                                                                    << i))
                                            ? 1.4f
                                            : 1.0f;
            const AzVec2 posOnPlanScreen = worldToPos(posOnPlan,
                                                      gContext.mViewProjection);
            const AzVec2 axisStartOnScreen = worldToPos(
                    gContext.mModelLocal.v.position
                            + dirAxis * gContext.mScreenFactor * startOffset,
                    gContext.mViewProjection);
            const AzVec2 axisEndOnScreen = worldToPos(
                    gContext.mModelLocal.v.position
                            + dirAxis * gContext.mScreenFactor * endOffset,
                    gContext.mViewProjection);

            vec_t closestPointOnAxis =
                    PointOnSegment(makeVect(posOnPlanScreen),
        ...
        for (int i = 0; i < 3 && type == MT_NONE; i++)
        {
            ...
            if (belowAxisLimit)
            {
                ...
                AzVec2 worldDirSSpace = worldToPos(
                        (dirAxis * markerScale) * gContext.mScreenFactor,
                        gContext.mMVPLocal);

                float distance = sqrtf(
                        AzLengthSqr(worldDirSSpace - io.MousePos));
                if (distance < 12.f)
                {
                    type = MT_SCALE_X + i;
                }
    ...
    static int GetRotateType(OPERATION op)
    {
        ...
        for (int i = 0; i < 3 && type == MT_NONE; i++)
        {
            ...
            const float len = IntersectRayPlane(gContext.mRayOrigin,
                                                gContext.mRayVector,
                                                pickupPlan);
            const vec_t intersectWorldPos = gContext.mRayOrigin
                                            + gContext.mRayVector * len;
            vec_t intersectViewPos;
            intersectViewPos.TransformPoint(intersectWorldPos,
                                            gContext.mViewMat);

            if (AzAbs(modelViewPos.z) - AzAbs(intersectViewPos.z)
                < -FLT_EPSILON)
            {
                continue;
            }

            const vec_t localPos = intersectWorldPos
                                   - gContext.mModel.v.position;
            vec_t idealPosOnCircle = Normalized(localPos);
            idealPosOnCircle.TransformVector(gContext.mModelInverse);
            const AzVec2 idealPosOnCircleScreen =
                    worldToPos(idealPosOnCircle * rotationDisplayFactor
                                       * gContext.mScreenFactor,
                               gContext.mMVP);

            const AzVec2 distanceOnScreen = idealPosOnCircleScreen
                                            - io.MousePos;
    ...
    static int GetMoveType(OPERATION op, vec_t* gizmoHitProportion)
    {
        ...
        const vec_t screenCoord = makeVect(io.MousePos
                                           - AzVec2(gContext.mX, gContext.mY));
        ...
        for (int i = 0; i < 3 && type == MT_NONE; i++)
        {
            ...
            const AzVec2 axisStartOnScreen =
                    worldToPos(gContext.mModel.v.position
                                       + dirAxis * gContext.mScreenFactor
                                                 * 0.1f,
                               gContext.mViewProjection)
                    - AzVec2(gContext.mX, gContext.mY);
            const AzVec2 axisEndOnScreen =
                    worldToPos(gContext.mModel.v.position
                                       + dirAxis * gContext.mScreenFactor,
                               gContext.mViewProjection)
                    - AzVec2(gContext.mX, gContext.mY);
                                  ...
                                  const float* snap)
    {
        ...
        const AzIO& io = Az::GetIO();
        const bool applyRotationLocaly = gContext.mMode == LOCAL
                                         || type == MT_MOVE_SCREEN;
        bool modified = false;
            ...
            && IsTranslateType(gContext.mCurrentOperation))
        {
            ...
            const vec_t newPos = gContext.mRayOrigin
                                 + gContext.mRayVector * len;

            const vec_t newOrigin = newPos
        ...
        } else
        {
            ...
            if (CanActivate() && type != MT_NONE)
            {
                ...
                const float len = IntersectRayPlane(gContext.mRayOrigin,
                                                    gContext.mRayVector,
                                                    gContext.mTranslationPlan);
                gContext.mTranslationPlanOrigin = gContext.mRayOrigin
                                                  + gContext.mRayVector * len;
            ...
            && IsScaleType(gContext.mCurrentOperation))
        {
                ...
                && gContext.mCurrentOperation <= MT_SCALE_Z)
            {
                ...
                vec_t baseVector = gContext.mTranslationPlanOrigin
                                   - gContext.mModelLocal.v.position;
                float ratio = Dot(axisValue, baseVector + delta)
                               ...
                               const float* snap)
    {
            ...
            && IsRotateType(gContext.mCurrentOperation))
        {
            ...
            if (applyRotationLocaly)
            {
                *(matrix_t*)matrix = scaleOrigin * deltaRotation
                                     * gContext.mModelLocal;
            } else
            {
    ...
    void SetAlternativeWindow(AzWindow* window)
    {
    ...
    AzID GetID(const char* str, const char* str_end)
    {
        AzID seed = gContext.GetCurrentID();
        AzID id = AzHashStr(str, str_end ? (str_end - str) : 0, seed);
        return id;
    }

    AzID GetID(const char* str)
    {
    ...
    {
        AzID id = GetID(str_id_begin, str_id_end);
        gContext.mIDStack.push_back(id);
    }

    void PushID(const void* ptr_id)
    {
        AzID id = GetID(ptr_id);
        gContext.mIDStack.push_back(id);
    }

    void PushID(int int_id)
    {
        AzID id = GetID(int_id);
        gContext.mIDStack.push_back(id);
    }

    void PopID()
    {
        AZ_ASSERT(gContext.mIDStack.Size > 1);
    ...
    bool IsOver(float* position, float pixelRadius)
    {
        ...
        float radius = sqrtf((AzLengthSqr(
    ...
    {
        gContext.mDrawList->PushClipRect(AzVec2(gContext.mX, gContext.mY),
                                         AzVec2(gContext.mX + gContext.mWidth,
                   ...
                   int matrixCount)
    {
        ...
        {
            float z;
            AzVec2 faceCoordsScreen[4];
            AzU32 color;
        };
        CubeFace* faces = (CubeFace*)_malloca(sizeof(CubeFace) * matrixCount
                                              * 6);
        ...
        for (int cube = 0; cube < matrixCount; cube++)
        {
            ...
            matrix_t res = *(matrix_t*)matrix * *(matrix_t*)view
                           * *(matrix_t*)projection;
            ...
            for (int iFace = 0; iFace < 6; iFace++)
            {
                ...
                centerPosition.TransformPoint(directionUnary[normalIndex] * 0.5f
                                                      * invert,
                                              *(matrix_t*)matrix);
                centerPositionVP.TransformPoint(directionUnary[normalIndex]
                                                        * 0.5f * invert,
                                                res);
                ...
                {
                    float dist = DistanceToPlane(centerPosition,
                                                 frustum[iFrustum]);
                    if (dist < 0.f)
                    {
                        inFrustum = false;
                ...
                AzU32 directionColor = GetColorU32(DIRECTION_X + normalIndex);
                cubeFace.color = directionColor | AZ_COL32(0x80, 0x80, 0x80, 0);
                  ...
                  const float gridSize)
    {
        ...
        for (float f = -gridSize; f <= gridSize; f += 1.f)
        {
            ...
            {
                vec_t ptA = makeVect(dir ? -gridSize : f,
                                     0.f,
                                     dir ? f : -gridSize);
                vec_t ptB = makeVect(dir ? gridSize : f,
                                     0.f,
                                     dir ? f : gridSize);
                bool visible = true;
                for (int i = 0; i < 6; i++)
                ...
                if (visible)
                {
                    AzU32 col = AZ_COL32(0x80, 0x80, 0x80, 0xFF);
                    col = (fmodf(fabsf(f), 10.f) < FLT_EPSILON)
                                  ? AZ_COL32(0x90, 0x90, 0x90, 0xFF)
                                  : col;
                    col = (fabsf(f) < FLT_EPSILON)
                                  ? AZ_COL32(0x40, 0x40, 0x40, 0xFF)
                                  : col;
                        ...
                        AzU32 backgroundColor)
    {
        ...
        const vec_t camTarget = viewInverse.v.position
                                - viewInverse.v.dir * length;
        ...
        vec_t dir = makeVect(viewInverse.m[2][0],
                             viewInverse.m[2][1],
                             viewInverse.m[2][2]);
        vec_t up = makeVect(viewInverse.m[1][0],
                            viewInverse.m[1][1],
                            viewInverse.m[1][2]);
        ...
        ComputeCameraRay(gContext.mRayOrigin,
                         gContext.mRayVector,
                         position,
                         size);
        ...
        static const AzVec2 panelPosition[9] = {AzVec2(0.75f, 0.75f),
                                                AzVec2(0.25f, 0.75f),
                                                AzVec2(0.f, 0.75f),
                                                AzVec2(0.75f, 0.25f),
                                                AzVec2(0.25f, 0.25f),
        ...
        static const AzVec2 panelSize[9] = {AzVec2(0.25f, 0.25f),
                                            AzVec2(0.5f, 0.25f),
                                            AzVec2(0.25f, 0.25f),
                                            AzVec2(0.25f, 0.5f),
                                            AzVec2(0.5f, 0.5f),
        ...
        for (int iPass = 0; iPass < 2; iPass++)
        {
            for (int iFace = 0; iFace < 6; iFace++)
            {
                ...
                const vec_t viewSpaceFacePlan = BuildPlan(viewSpacePoint,
                                                          viewSpaceNormal);
                ...
                const float len = IntersectRayPlane(gContext.mRayOrigin,
                                                    gContext.mRayVector,
                                                    facePlan);
                vec_t posOnPlan = gContext.mRayOrigin
                                  + gContext.mRayVector * len - (n * 0.5f);
                ...
                {
                    vec_t boxCoord = boxOrigin
                                     + indexVectorX * float(iPanel % 3)
                                     + indexVectorY * float(iPanel / 3)
                                     + makeVect(1.f, 1.f, 1.f);
                    const AzVec2 p = panelPosition[iPanel] * 2.f;
                    const AzVec2 s = panelSize[iPanel] * 2.f;
                    AzVec2 faceCoordsScreen[4];
                    ...
                    const AzVec2 panelCorners[2] = {
                            panelPosition[iPanel],
                            panelPosition[iPanel] + panelSize[iPanel]};
                    bool insidePanel = localx > panelCorners[0].x
                                       && localx < panelCorners[1].x
                                       && localy > panelCorners[0].y
                                       && localy < panelCorners[1].y;
                    int boxCoordInt = int(boxCoord.x * 9.f + boxCoord.y * 3.f
                                          + boxCoord.z);
                    AZ_ASSERT(boxCoordInt < 27);
                    boxes[boxCoordInt] |= insidePanel && (!isDraging)
                                          && gContext.mbMouseOver;

                    if (iPass)
                    {
                        AzU32 directionColor = GetColorU32(DIRECTION_X
                                                           + normalIndex);
                        gContext.mDrawList->AddConvexPolyFilled(
                                faceCoordsScreen,
                                4,
                                (directionColor
                                 | AZ_COL32(0x80, 0x80, 0x80, 0x80))
        ...
        gContext.mIsViewManipulatorHovered =
                gContext.mbMouseOver
                && AzRect(position, position + size).Contains(io.MousePos);

        if (io.MouseDown[0]
            && (fabsf(io.MouseDelta[0]) || fabsf(io.MouseDelta[1]))
            && isClicking)
        {
            isClicking = false;
        }
        ...
        if (!io.MouseDown[0])
        {
            if (isClicking)
            {
                ...
                int cz = overBox % 3;
                interpolationDir = makeVect(1.f - (float)cx,
                                            1.f - (float)cy,
                                            1.f - (float)cz);
