...
namespace AzGuizmo
{
    ...
    static AzVec2 operator*(const AzVec2& a, const float b)
    {
        return AzVec2(a.x * b, a.y * b);
    }
#endif

    static float smoothstep(float edge0, float edge1, float x)
    {
        x = AzClamp((x - edge0) / (edge1 - edge0), 0.0f, 1.0f);
        return x * x * (3 - 2 * x);
    }
                         ...
                         bool edited)
    {
        ...
        static const AzVec2 localOffsets[4] = {AzVec2(1, 0),
                                               AzVec2(0, 1),
                                               AzVec2(-1, 0),
                                               AzVec2(0, -1)};
        AzVec2 offsets[4];
        ...
        const AzVec2 center = pos * size + offset;
        const AzRect anchor(center - AzVec2(5, 5), center + AzVec2(5, 5));
    ...
    int Edit(Delegate& delegate,
             const AzVec2& size,
             unsigned int id,
             const AzRect* clippingRect,
             AzVector<EditPoint>* selectedPoints)
    {
        static bool selectingQuad = false;
        static AzVec2 quadSelection;
        ...
        Az::BeginChild(id, size, AzChildFlags_FrameStyle);
        delegate.focused = Az::IsWindowFocused();
        AzDrawList* draw_list = Az::GetWindowDrawList();
        if (clippingRect)
            draw_list->PushClipRect(clippingRect->Min, clippingRect->Max, true);

        const AzVec2 offset = Az::GetCursorScreenPos() + AzVec2(0.f, size.y);
        const AzVec2 ssize(size.x, -size.y);
        const AzRect container(offset + AzVec2(0.f, ssize.y),
                               offset + AzVec2(ssize.x, 0.f));
        AzVec2& min = delegate.GetMin();
        AzVec2& max = delegate.GetMax();
        ...
        if (container.Contains(io.MousePos))
        {
            if (fabsf(io.MouseWheel) > FLT_EPSILON)
            {
                ...
                float ratioY = AzLerp(min.y, max.y, r);
            ...
            if (!scrollingV && Az::IsMouseDown(2))
            {
                scrollingV = true;
            }
        }
        AzVec2 range = max - min + AzVec2(1.f, 0.f);

        const AzVec2 viewSize(size.x, -size.y);
        const AzVec2 sizeOfPixel = AzVec2(1.f, 1.f) / viewSize;
        ...
        draw_list->AddRectFilled(offset,
                                 offset + ssize,
                                 delegate.GetBackgroundColor());

        auto pointToRange = [&](AzVec2 pt) { return (pt - min) / range; };
        auto rangeToPoint = [&](AzVec2 pt) { return (pt * range) + min; };

        draw_list->AddLine(AzVec2(-1.f, -min.y / range.y) * viewSize + offset,
                           AzVec2(1.f, -min.y / range.y) * viewSize + offset,
                           0xFF000000,
                           1.5f);
        ...
        {
            AzSwap(curvesIndex[overCurve], curvesIndex[curveCount - 1]);
        ...
        for (size_t cur = 0; cur < curveCount; cur++)
        {
            ...
            if (curveType == CurveNone)
                continue;
            const AzVec2* pts = delegate.GetPoints(c);
            uint32_t curveColor = delegate.GetCurveColor(c);
            if ((c == highLightedCurveIndex && selection.empty()
                 && !selectingQuad)
                || movingCurve == c)
            ...
            {
                const AzVec2 p1 = pointToRange(pts[p]);
                const AzVec2 p2 = pointToRange(pts[p + 1]);
                ...
                if (curveType == CurveSmooth || curveType == CurveLinear)
                {
                    ...
                    for (size_t substep = 0; substep < subStepCount - 1;
                         substep++)
                    {
                        float t = float(substep) * step;

                        const AzVec2 sp1 = AzLerp(p1, p2, t);
                        const AzVec2 sp2 = AzLerp(p1, p2, t + step);

                        const float rt1 = smoothstep(p1.x, p2.x, sp1.x);
                        const float rt2 = smoothstep(p1.x, p2.x, sp2.x);

                        const AzVec2 pos1 = AzVec2(sp1.x,
                                                   AzLerp(p1.y, p2.y, rt1))
                                                    * viewSize
                                            + offset;
                        const AzVec2 pos2 = AzVec2(sp2.x,
                                                   AzLerp(p1.y, p2.y, rt2))
                                                    * viewSize
                                            + offset;

                        if (distance(io.MousePos.x,
                                     io.MousePos.y,
                                     pos1.x,
                                     pos1.y,
                                     pos2.x,
                ...
                {
                    AzVec2 dp1 = p1 * viewSize + offset;
                    AzVec2 dp2 = AzVec2(p2.x, p1.y) * viewSize + offset;
                    AzVec2 dp3 = p2 * viewSize + offset;
        ...
        static AzVec2 mousePosOrigin;
        static std::vector<AzVec2> originalPoints;
        if (overSelectedPoint && io.MouseDown[0])
        {
                ...
                && !selection.empty())
            {
                if (!pointsMoved)
                {
                    ...
                    {
                        const AzVec2* pts = delegate.GetPoints(sel.curveIndex);
                ...
                {
                    const AzVec2 p = rangeToPoint(
                            pointToRange(originalPoints[originalIndex])
                            + (io.MousePos - mousePosOrigin) * sizeOfPixel);
                    const int newIndex = delegate.EditPoint(sel.curveIndex,
                                                            sel.pointIndex,
                                                            p);
        ...
        {
            const AzVec2 np = rangeToPoint((io.MousePos - offset) / viewSize);
        ...
        if (movingCurve != -1)
        {
            ...
            const AzVec2* pts = delegate.GetPoints(movingCurve);
            if (!pointsMoved)
            {
                mousePosOrigin = io.MousePos;
                pointsMoved = true;
            ...
            if (ptCount >= 1)
            {
                for (size_t p = 0; p < ptCount; p++)
                {
                    delegate.EditPoint(movingCurve,
                                       int(p),
                                       rangeToPoint(
                                               pointToRange(originalPoints[p])
                                               + (io.MousePos - mousePosOrigin)
                                                         * sizeOfPixel));
                }
                ret = 1;
            }
        ...
        if (movingCurve == -1 && overCurve != -1 && Az::IsMouseClicked(0)
        ...
        if (selectingQuad)
        {
            const AzVec2 bmin = AzMin(quadSelection, io.MousePos);
            const AzVec2 bmax = AzMax(quadSelection, io.MousePos);
            draw_list->AddRectFilled(bmin, bmax, 0x40FF0000, 1.f);
            draw_list->AddRect(bmin, bmax, 0xFFFF0000, 1.f);
            const AzRect selectionQuad(bmin, bmax);
            if (!io.MouseDown[0])
            {
                if (!io.KeyShift)
                    selection.clear();
                ...
                for (size_t c = 0; c < curveCount; c++)
                {
                    ...
                    for (size_t p = 0; p < ptCount; p++)
                    {
                        const AzVec2 center = pointToRange(pts[p]) * viewSize
                                              + offset;
        ...
        if (!overCurveOrPoint && Az::IsMouseClicked(0) && !selectingQuad
