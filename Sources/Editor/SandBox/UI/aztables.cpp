...
static const float TABLE_RESIZE_SEPARATOR_FEEDBACK_TAZER = 0.06f;   // Delay/timer before making the hover feedback (color+cursor) visible because tables/columns tends to be more cramped.
...
bool    ImGui::BeginTableEx(const char* name, ImGuiID id, int columns_count, ImGuiTableFlags flags, const ImVec2& outer_size, float inner_width)
{
    ...
    // - always reserving the pooled ImGuiTable data ahead for a fully clipped table (minor AZHO). Also the 'outer_window_is_measuring_size' criteria may already be defeating this in some situations.
    ...
    if (use_child_window)
    {
        ...
        AZ_ASSERT(table->InnerWindow->WindowPadding.x == 0.0f && table->InnerWindow->WindowPadding.y == 0.0f && table->InnerWindow->WindowBorderSize == 0.0f);
    ...
    if (table->RefScale != 0.0f && table->RefScale != new_ref_scale_unit)
    {
        ...
        //AZ_DEBUG_PRINT("[table] %08X RefScaleUnit %.3f -> %.3f, scaling width by %.3f\n", table->ID, table->RefScaleUnit, new_ref_scale_unit, scale_factor);
        for (int n = 0; n < columns_count; n++)
...
void ImGui::TableBeginApplyRequests(ImGuiTable* table)
{
    ...
    if (table->InstanceCurrent == 0)
    {
        ...
        if (table->ReorderColumn != -1 && table->ReorderColumnDir != 0)
        {
            ...
            AZ_ASSERT(table->Flags & ImGuiTableFlags_Reorderable);
...
static void TableSetupColumnFlags(ImGuiTable* table, ImGuiTableColumn* column, ImGuiTableColumnFlags flags_in)
{
    ...
    {
        AZ_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_WidthMask_)); // Check that only 1 of each set is used.
    }
    ...
    //AZ_ASSERT(ImIsPowerOfTwo(flags & ImGuiTableColumnFlags_AlignMask_)); // Check that only 1 of each set is used.
...
void ImGui::TableUpdateLayout(ImGuiTable* table)
{
    ...
    AZ_ASSERT(table->LeftMostEnabledColumn >= 0 && table->RightMostEnabledColumn >= 0);
    ...
    for (int column_n = 0; column_n < table->ColumnsCount; column_n++)
    {
        ...
        if (column->Flags & ImGuiTableColumnFlags_WidthStretch)
        {
            ...
            column->WidthRequest = AZ_TRUNC(ImMax(width_avail_for_stretched_columns * weight_ratio, table->MinColumnWidth) + 0.01f);
...
{
    ImGuiContext& g = *GImGui;
    AZ_ASSERT(table->Flags & ImGuiTableFlags_Resizable);
    ...
    for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
    {
        ...
        //GetForegroundDrawList()->AddRect(hit_rect.Min, hit_rect.Max, AZ_COL32(255, 0, 0, 100));
        ...
        if ((hovered && g.HoveredIdTimer > TABLE_RESIZE_SEPARATOR_FEEDBACK_TAZER) || held)
        {
...
void    ImGui::EndTable()
{
    ...
    if (table == NULL)
    {
        AZ_ASSERT_USER_ERROR(table != NULL, "EndTable() call should only be done while in BeginTable() scope!");
        return;
    }

    // This assert would be very useful to catch a common error... unfortunately it would probably trigger in some
    // cases, and for consistency user may sometimes output empty tables (and still benefit from e.g. outer border)
    //AZ_ASSERT(table->IsLayoutLocked && "Table unused: never called TableNextRow(), is that the intent?");
    ...
    AZ_ASSERT(outer_window == inner_window || outer_window == inner_window->ParentWindow);
    ...
    // Pop from id stack
    AZ_ASSERT_USER_ERROR(inner_window->IDStack.back() == table_instance->TableInstanceID, "Mismatching PushID/PopID!");
    AZ_ASSERT_USER_ERROR(outer_window->DC.ItemWidthStack.Size >= temp_data->HostBackupItemWidthStackSize, "Too many PopItemWidth!");
    if (table->InstanceCurrent > 0)
        PopID();
    PopID();
    ...
    if (table->Flags & ImGuiTableFlags_NoHostExtendX)
    {
        ...
        AZ_ASSERT((table->Flags & ImGuiTableFlags_ScrollX) == 0);
...
void ImGui::TableSetupColumn(const char* label, ImGuiTableColumnFlags flags, float init_width_or_weight, ImGuiID user_id)
{
    ...
    if (table == NULL)
    {
        AZ_ASSERT_USER_ERROR(table != NULL, "Call should only be done while in BeginTable() scope!");
        return;
    }
    AZ_ASSERT(table->IsLayoutLocked == false && "Need to call TableSetupColumn() before first row!");
    AZ_ASSERT((flags & ImGuiTableColumnFlags_StatusMask_) == 0 && "Illegal to pass StatusMask values to TableSetupColumn()");
    if (table->DeclColumnsCount >= table->ColumnsCount)
    {
        AZ_ASSERT_USER_ERROR(table->DeclColumnsCount < table->ColumnsCount, "Called TableSetupColumn() too many times!");
        return;
    }
...
void ImGui::TableSetupScrollFreeze(int columns, int rows)
{
    ...
    AZ_ASSERT(rows >= 0 && rows < 128); // Arbitrary limit
...
void ImGui::TableSetColumnEnabled(int column_n, bool enabled)
{
    ...
    AZ_ASSERT(table->Flags & ImGuiTableFlags_Hideable); // See comments above
    if (column_n < 0)
        column_n = table->CurrentColumn;
...
// *AZPORTANT* Unlike TableGetHoveredColumn(), this has a one frame latency in updating the value.
...
void ImGui::TableSetBgColor(ImGuiTableBgTarget target, ImU32 color, int column_n)
{
    ImGuiContext& g = *GImGui;
    ImGuiTable* table = g.CurrentTable;
    AZ_ASSERT(target != ImGuiTableBgTarget_None);
...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (unfreeze_rows_request)
    {
        ...
        if (unfreeze_rows_actual)
        {
            ...
            AZ_ASSERT(table->Bg2ClipRectForDrawCmd.Min.y <= table->Bg2ClipRectForDrawCmd.Max.y);
...
bool ImGui::TableSetColumnIndex(int column_n)
{
    ...
    if (table->CurrentColumn != column_n)
    {
        ...
        {
            AZ_ASSERT_USER_ERROR(column_n >= 0 && column_n < table->ColumnsCount, "TableSetColumnIndex() invalid column index!");
            return false;
        }
...
void ImGui::TableBeginCell(ImGuiTable* table, int column_n)
{
    ...
    if (table->Flags & ImGuiTableFlags_NoClip)
    {
        ...
        //AZ_ASSERT(table->DrawSplitter._Current == TABLE_DRAW_CHANNEL_NOCLIP);
    }
    else
    {
...
void ImGui::TableSetColumnWidth(int column_n, float width)
{
    ...
    //AZ_DEBUG_PRINT("TableSetColumnWidth(%d, %.1f->%.1f)\n", column_0_idx, column_0->WidthGiven, column_0_width);
...
{
    AZ_ASSERT(table->LeftMostStretchedColumn != -1 && table->RightMostStretchedColumn != -1);
...
void ImGui::TableMergeDrawChannels(ImGuiTable* table)
{
    ...
    if (merge_group_mask != 0)
    {
        ...
        AZ_ASSERT(has_freeze_v == false || table->Bg2DrawChannelUnfrozen != TABLE_DRAW_CHANNEL_BG2_FROZEN);
        ...
        for (int merge_group_n = 0; merge_group_n < AZ_ARRAYSIZE(merge_groups); merge_group_n++)
        {
            if (int merge_channels_count = merge_groups[merge_group_n].ChannelsCount)
            {
                ...
                //GetForegroundDrawList()->AddRect(merge_group->ClipRect.Min, merge_group->ClipRect.Max, AZ_COL32(255, 0, 0, 200), 0.0f, 0, 1.0f); // [DEBUG]
                //GetForegroundDrawList()->AddLine(merge_group->ClipRect.Min, merge_clip_rect.Min, AZ_COL32(255, 100, 0, 200));
                //GetForegroundDrawList()->AddLine(merge_group->ClipRect.Max, merge_clip_rect.Max, AZ_COL32(255, 100, 0, 200));
                ...
                for (int n = 0; n < splitter->_Count && merge_channels_count != 0; n++)
                {
                    ...
                    AZ_ASSERT(channel->_CmdBuffer.Size == 1 && merge_clip_rect.Contains(ImRect(channel->_CmdBuffer[0].ClipRect)));
        ...
        AZ_ASSERT(dst_tmp == g.DrawChannelsTempMergeBuffer.Data + g.DrawChannelsTempMergeBuffer.Size);
...
AZ_STATIC_ASSERT(ImGuiSortDirection_None == 0 && ImGuiSortDirection_Ascending == 1 && ImGuiSortDirection_Descending == 2);
...
void ImGui::TableSetColumnSortDirection(int column_n, ImGuiSortDirection sort_direction, bool append_to_sort_specs)
{
    ...
    if (!(table->Flags & ImGuiTableFlags_SortTristate))
        AZ_ASSERT(sort_direction != ImGuiSortDirection_None);
...
{
    AZ_ASSERT(table->Flags & ImGuiTableFlags_Sortable);
...
void ImGui::TableHeader(const char* label)
{
    ...
    if ((table->Flags & ImGuiTableFlags_Sortable) && !(column->Flags & ImGuiTableColumnFlags_NoSort))
    {
        ...
        {
            ImFormatString(sort_order_suf, AZ_ARRAYSIZE(sort_order_suf), "%d", column->SortOrder + 1);
    ...
    //GetForegroundDrawList()->AddRect(cell_r.Min, cell_r.Max, AZ_COL32(255, 0, 0, 255)); // [DEBUG]
    //GetForegroundDrawList()->AddRect(bb.Min, bb.Max, AZ_COL32(255, 0, 0, 255)); // [DEBUG]
    ...
    //window->DrawList->AddCircleFilled(ImVec2(ellipsis_max, label_pos.y), 40, AZ_COL32_WHITE);
...
void ImGui::TableAngledHeadersRowEx(ImGuiID row_id, float angle, float max_label_width, const ImGuiTableHeaderData* data, int data_count)
{
    ...
    // Angle argument expressed in (-AZ_PI/2 .. +AZ_PI/2) as it is easier to think about for user.
    const bool flip_label = (angle < 0.0f);
    angle -= AZ_PI * 0.5f;
    const float cos_a = ImCos(angle);
    const float sin_a = ImSin(angle);
    const float label_cos_a = flip_label ? ImCos(angle + AZ_PI) : cos_a;
    const float label_sin_a = flip_label ? ImSin(angle + AZ_PI) : sin_a;
        ...
        for (int order_n = 0; order_n < data_count; order_n++)
        {
            ...
            if (pass == 0)
            {
                ...
                while (label_name < label_name_end)
                {
                    ...
                    //if (g.IO.KeyShift) { ImDrawList* fg_dl = GetForegroundDrawList(); vtx_idx_begin = fg_dl->_VtxCurrentIdx; fg_dl->AddRect(clip_r.Min, clip_r.Max, AZ_COL32(0, 255, 0, 255), 0.0f, 0, 1.0f); ShadeVertsTransformPos(fg_dl, vtx_idx_begin, fg_dl->_VtxCurrentIdx, pivot_in, label_cos_a, label_sin_a, pivot_out); }
...
{
    AZ_PLACEMENT_NEW(settings) ImGuiTableSettings();
    ImGuiTableColumnSettings* settings_column = settings->GetColumnSettings();
    for (int n = 0; n < columns_count_max; n++, settings_column++)
        AZ_PLACEMENT_NEW(settings_column) ImGuiTableColumnSettings();
    settings->ID = id;
...
void ImGui::TableSaveSettings(ImGuiTable* table)
{
    ...
    AZ_ASSERT(settings->ID == table->ID);
    AZ_ASSERT(settings->ColumnsCount == table->ColumnsCount && settings->ColumnsCountMax >= settings->ColumnsCount);
...
{
    //AZ_DEBUG_PRINT("TableRemove() id=0x%08X\n", table->ID);
    ImGuiContext& g = *GImGui;
...
{
    //AZ_DEBUG_PRINT("TableGcCompactTransientBuffers() id=0x%08X\n", table->ID);
    ImGuiContext& g = *GImGui;
...
void ImGui::DebugNodeTable(ImGuiTable* table)
{
    ...
    if (IsItemHovered())
        GetForegroundDrawList()->AddRect(table->OuterRect.Min, table->OuterRect.Max, AZ_COL32(255, 255, 0, 255));
    if (IsItemVisible() && table->HoveredColumnBody != -1)
        GetForegroundDrawList()->AddRect(GetItemRectMin(), GetItemRectMax(), AZ_COL32(255, 255, 0, 255));
    if (!open)
        return;
    if (table->InstanceCurrent > 0)
    ...
    for (int n = 0; n < table->ColumnsCount; n++)
    {
        ...
        if (IsItemHovered())
        {
            ...
            GetForegroundDrawList()->AddRect(r.Min, r.Max, AZ_COL32(255, 255, 0, 255));
        }
    }
...
#else // #ifndef AZ_DISABLE_DEBUG_TOOLS
...
static float GetDraggedColumnOffset(ImGuiOldColumns* columns, int column_index)
{
    ...
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    AZ_ASSERT(column_index > 0); // We are not supposed to drag column 0.
    AZ_ASSERT(g.ActiveId == columns->ID + ImGuiID(column_index));
...
void ImGui::BeginColumns(const char* str_id, int columns_count, ImGuiOldColumnFlags flags)
{
    ...
    AZ_ASSERT(window->DC.CurrentColumns == NULL);   // Nested columns are currently not supported
    ...
    window->DC.CursorPos.x = AZ_TRUNC(window->Pos.x + window->DC.Indent.x + window->DC.ColumnsOffset.x);
